...
const UpdateProduct = ({match}) => {
    ...
    const clickSubmit = (event) =>{
        ...
        updateProduct(match.params.productId,user._id,token,formData).then(data => {
            if(data.error) setValues({...values,error: data.error});
            else setValues({...values,name:'',description:'',photo:'',price:'',quanity:'',loading:false, error: false, redirectToProfile: true,createdProduct: data.name});
        });
    };
    const newPostForm = () => (
    ...
    const showSuccess = () => (
        <div classname = "alert alert-info" style ={{display: createdProduct ? '': 'none'}}><h2>{`${createdProduct}`} is updated!</h2></div>
    );
