...
if(orders.length > 0) return (<h1 className = "text-danger display-2"> Total orders: {orders.length}</h1>)
...
const handleStatusChange = (e,orderId) => {
    updateOrderStatus(user._id,token,orderId,e.target.value).then(data => {
        if(data.error) console.log("Status update fail");
        else loadOrders();
    });
};
...
return (
    <Layout title = "Orders" description = {`Ready to manage all the orders?`}>
       <div className = "row">
